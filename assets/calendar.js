function CalendarApp(date, apts) {
  ...
  this.quotes = [];
  this.apts = apts;
  this.aptDates = apts.map(apt => apt.day.toString());
  this.eles = {
  };
  ...
  this.todayIsSpan.textContent = "Heute ist " + this.months[date.getMonth()] + " " + date.getDate();
}
...
CalendarApp.prototype.openDayWindow = function(date){
  ...
  var _dayTopbarText = "Am " + this.months[day.getMonth()] + " " + day.getDate() + ", " + day.getFullYear();
  if ( day < new Date(now.getFullYear(), now.getMonth(), now.getDate())) {
    _dayTopbarText += " gab es ";
  } else {
     _dayTopbarText += " gibt es ";
  }

  var eventsToday = this.showEventsByDay(day);
  if ( !eventsToday ) {
    _dayTopbarText += "keine ";
  ...
  this.dayEventsEle.textContent = _dayTopbarText + "Einträge";
